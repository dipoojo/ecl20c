...
from utils.dataset import TrajectCR_Dataset, TrajectCR_Dataset_Dynamic
...
def create_source_target(timeseries, window_size, prediction_horizon):
    """
    Creates source and target data for time series forecasting using a sliding window.

    Args:
        timeseries (torch.Tensor): The time series data. numpy instead of touch
        window_size (int): The size of the sliding window.
        prediction_horizon (int): The number of steps to predict into the future.

    Returns:
        torch.Tensor, torch.Tensor: The source and target data. returns numpy instead of Torch
    """

    source_data = []
    target_data = []

    for i in range(len(timeseries) - window_size - prediction_horizon + 1):
        source = timeseries[i: i + window_size]
        target = timeseries[i + window_size: i + window_size + prediction_horizon]

        source_data.append(source)
        target_data.append(target)

    return np.stack(source_data), np.stack(target_data)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename='result/result_log.txt', level=logging.INFO)
    ...
    optactdict = defaultdict(list)
    xtildict = defaultdict(list)
    optcostdict = defaultdict(list)
    ...
    epoch_number = 2 #80 journal change
    lr = 4e-20
    ...
    for input_dict in [ecl20]: # [pureml, robd, mlrobd, ecl20, ecl20b]:    
        ...
        hidden_size = 96 #changed from 10 to 24
        hidden_two = 24
        num_classes = 1

        input_size = 1   # changed to 1 for seq to seq 1 * num_classes #change input size to 1 * instead of 2*
        seq_length = 24 #25 #25  journal change 25 to 24
        num_layers = 3
        ...
        #train_seq = sliding_windows(train_raw, seq_length) new LSTM equence
        train_seq, train_target_seq = create_source_target(train_raw, seq_length, seq_length)
        
        #val_seq = sliding_windows(val_raw, seq_length) new LSTM sequence
        val_seq, val_target_seq = create_source_target(val_raw, seq_length, seq_length)
        ...
        for i in range(1,5):
            ...
            temp_seq[i], temp_target_seq[i] = create_source_target(test_raw[i], seq_length, seq_length)

            op_temp_cost_array[i] = optimal_cost_calc(test_raw[i][seq_length:], seq_length)
        ...
        traject_dataset_train = TrajectCR_Dataset_Dynamic(train_seq, train_target_seq, w, mute=mute)
        train_dataloader = DataLoader(traject_dataset_train, batch_size=batch_size, shuffle=False, num_workers=4)

        traject_dataset_val = TrajectCR_Dataset_Dynamic(val_seq, val_target_seq, w, mute=mute)
        ...
        for i in range(1,5):
            traject_dataset_temp[i] = TrajectCR_Dataset_Dynamic(temp_seq[i], temp_target_seq[i], w, mute=mute)
        ...
        lstm_dynamic = LSTM_unroll_dynamic_seq(num_classes, input_size, hidden_size, num_layers, 
                        seq_length, hidden_two, w, l_1, l_2, l_3)
        ...
        train_cr_vals_dynamic(lstm_dynamic, optimizer_dy, train_dataloader, val_dataloader,temp_dataloader, temp_seq,
                              ...
                              df_train_dy, df_test_dy, df_val_dy, df_testp, outdict, p2dict,tempdict, optactdict,xtildict, optcostdict, mtl_weight=input_dict["mtl_weight"], mute=mute, 
    ...
    df_testp.to_csv('result/df_testpone_five')
    with open('result/outdict.csv', 'w') as csv_file:  
        writer = csv.writer(csv_file)
        for key, value in outdict.items():
           writer.writerow([key, value])
    with open('result/psdict.csv', 'w') as csv_file:  
       writer = csv.writer(csv_file)
       for key, value in p2dict.items():
           writer.writerow([key, value])  

    with open('result/tempdict.csv', 'w') as csv_file:  
       writer = csv.writer(csv_file)
       for key, value in tempdict.items():
           writer.writerow([key, value])           

    with open('result/optactdict.csv', 'w') as csv_file:  
       writer = csv.writer(csv_file)
       for key, value in optactdict.items():
           writer.writerow([key, value])     


    with open('result/xtildict.csv', 'w') as csv_file:  
       writer = csv.writer(csv_file)
       for key, value in xtildict.items():
           writer.writerow([key, value])            


    with open('result/optcost.csv', 'w') as csv_file:  
       writer = csv.writer(csv_file)
       for key, value in optcostdict.items():
           writer.writerow([key, value])            
