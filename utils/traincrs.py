...
def train_cr_vals_dynamic(ml_model, optimizer, train_dataloader, val_dataloader,temp_dataloader,
            ...
            df_train, df_test, df_val, df_testp, outdict, p2dict, tempdict, optactdict, xtildict, optcostdict, mtl_weight = 0.5, mute=True, l_1=1, l_2=1, l_3=1, calib=True):
    from utils.loss import object_loss_cost as objl
    from utils.loss import object_loss_cr as objlr
    global n_iter, n_iter_val, use_cuda
...
#updated names with lambdas for ease of tracking
        ...
        if epoch > num_epoch-3:
            for i in range(1,4): #journal change originally 5 changed to 4 to exclude all quarters
                ...
                #print(torch.max(hit_cost))
                outdict[f'{epoch}-{i}'].append(tmval.flatten().tolist())
                p2dict[f'{epoch}-{i}'].append(p2_test.flatten().tolist())
                tempdict[f'{epoch}-{i}'].append(list(temp_seq[i]))
                optactdict[f'{epoch}-{i}'].append(list(opt_action))
                xtildict[f'{epoch}-{i}'].append(list(xtild))
                optcostdict[f'{epoch}-{i}'].append(list(opt_cost))
