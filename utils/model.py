...
class LSTM_unroll_dynamic(nn.Module):
    def __init__(self, num_classes, input_size, hidden_size, num_layers, 
        ...
        self.lstm = nn.LSTM(input_size=input_size, hidden_size=hidden_size,
                            num_layers=num_layers, batch_first=True)
        self.fc = nn.Linear(hidden_size, input_size) #changed from number of classes to input size Seq to Seq ******
    ...
    def forward(self, x, y, p_1, mode="train", calib = True):
        h_0 = torch.zeros(
            self.num_layers, x.size(0), self.hidden_size, device = x.device)
        ...
        if calib:
            ...
            optimal_action, optimal_cost = calculate_offline_optimal_dynamic(x[:,1:,:], x[:,0,:], switch_weight=10)

            p_2 = torch.norm(out-optimal_action, dim=1)/optimal_cost
            ...
            self.l_2 = (2*self.l_1/(2*10))*(torch.sqrt((1+(10/2)*(self.l_3/self.l_1))**2 + 4*10**2/(10*2)) + 1 - 2/self.l_1 - (10/2)*(self.l_3/self.l_1))
                    ...
                    #if calib:
            action_0 = (1 + self.w*self.l_2)*x + self.w*self.l_1*y + self.w*self.l_3*out
            action_0 /= 1 + self.w*(self.l_1 + self.l_2 + self.l_3)
        ...
        final_out = torch.cat(action_sequence,1)
        '''
        return final_out, p_2, optimal_action, optimal_cost, out
...
class LSTM_unroll_dynamic_seq(nn.Module):
    def __init__(self, num_classes, input_size, hidden_size, num_layers, 
                 seq_length, hidden_four, w, l_1, l_2, l_3):
        ...
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU()
        #self.hidden_four = hidden_size/4
        ...
        #self.lstm = nn.LSTM(input_size=input_size, hidden_size=hidden_size,
        #                    num_layers=num_layers, batch_first=True)
        self.fc = nn.Linear(hidden_size, hidden_four) #changed from number of classes to input size Seq to Seq ******
        self.f2c = nn.Linear(hidden_four, input_size)
